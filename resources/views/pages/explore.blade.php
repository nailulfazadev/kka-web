...
@section('content')
    ...
    {{-- ===================== TYPE FILTER ===================== --}}
    <section class="mb-3 top-0">
        <div class="flex gap-2 px-5 overflow-x-auto hide-scrollbar">
            @php
                $typeOptions = [
                    'all'     => ['label' => 'Semua Kelas', 'icon' => 'fa-graduation-cap'],
                    'live'    => ['label' => 'Sesi Live', 'icon' => 'fa-video'],
                    'ecourse' => ['label' => 'Belajar Mandiri', 'icon' => 'fa-play-circle']
                ];
            @endphp
            @foreach($typeOptions as $key => $opt)
                @php
                    $isActive = request('type', 'all') === $key;
                @endphp
                <a href="{{ url('/pelatihan?type=' . $key . '&filter=' . request('filter', '') . '&pricing=' . request('pricing', '') . '&search=' . request('search', '')) }}"
                   class="flex-shrink-0 flex items-center gap-1.5 px-4 py-2.5 rounded-2xl text-[11px] font-bold transition-all
                   {{ $isActive ? 'bg-primary text-on-primary shadow-[0_4px_15px_rgba(106,242,222,0.25)] scale-105' : 'glass-card border border-white/5 text-on-surface hover:bg-white/5' }}">
                    <i class="fa-solid {{ $opt['icon'] }} text-[12px] {{ $isActive ? 'text-on-primary' : 'text-primary' }}"></i>
                    {{ $opt['label'] }}
                </a>
            @endforeach
        </div>
    ...
    <section class="mb-3">
        <div class="flex gap-2 px-5 overflow-x-auto hide-scrollbar">
            @foreach(['semua' => 'Semua Status', 'aktif' => 'Aktif', 'mendatang' => 'Mendatang', 'selesai' => 'Selesai'] as $key => $label)
                @php
                    $isActive = request('filter', '') === $key || (request('filter') === null && $key === 'semua');
                @endphp
                <a href="{{ url('/pelatihan?filter=' . ($key === 'semua' ? '' : $key) . '&pricing=' . request('pricing', '') . '&type=' . request('type', 'all') . '&search=' . request('search', '')) }}"
                   class="flex-shrink-0 px-4 py-2 rounded-2xl text-xs font-semibold transition-colors
                   {{ $isActive ? 'bg-surface-container-highest text-on-surface shadow-sm' : 'glass-card border border-white/5 text-on-surface-variant hover:text-on-surface' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
    ...
    <section class="mb-7">
        <div class="flex gap-2 px-5 overflow-x-auto hide-scrollbar">
            ...
            @foreach($pricingOptions as $key => $opt)
                ...
                @endphp
                <a href="{{ url('/pelatihan?pricing=' . ($key === 'all' ? '' : $key) . '&filter=' . request('filter', '') . '&type=' . request('type', 'all') . '&search=' . request('search', '')) }}"
                   ...
                   class="glass-card rounded-[1.75rem] overflow-hidden border border-white/5 block active:scale-[0.98] transition-transform">
                    ...
                    <div class="relative bg-surface-container-high">
                        ...
                        <div class="absolute top-3 left-3 flex gap-1.5">
                            ...
                            <span class="inline-flex items-center gap-1 bg-blue-500/80 text-white backdrop-blur-md text-[9px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow-md">
