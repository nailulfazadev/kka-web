...
<form method="POST" action="{{ route('admin.pelatihan.store') }}" enctype="multipart/form-data" class="max-w-2xl space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div><label class="text-sm font-medium text-on-surface-variant mb-1 block">Tanggal Mulai (Opsional utk E-Course)</label><input type="date" name="start_date" value="{{ old('start_date') }}" class="w-full bg-surface-container-highest border-none rounded-xl py-3 px-4 text-on-surface focus:ring-1 focus:ring-primary/40"></div>
        <div><label class="text-sm font-medium text-on-surface-variant mb-1 block">Tanggal Selesai (Opsional utk E-Course)</label><input type="date" name="end_date" value="{{ old('end_date') }}" class="w-full bg-surface-container-highest border-none rounded-xl py-3 px-4 text-on-surface focus:ring-1 focus:ring-primary/40"></div>
    </div>
    ...
    <div class="glass-card border border-primary/20 bg-primary/5 rounded-xl p-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-[10px] text-on-surface-variant">Jika diaktifkan, peserta bisa mengunduh sertifikat, undangan, rekap presensi, dan materi.</p>
        </div>
    ...
    <div class="glass-card border border-blue-500/20 bg-blue-500/5 rounded-xl p-4 flex items-center justify-between mt-4 mb-4">
        <div>
            <h4 class="font-bold text-on-surface text-sm">Jadikan E-Course Mandiri</h4>
            <p class="text-[10px] text-on-surface-variant">Jika diaktifkan, pelatihan ini menjadi produk rekaman mandiri. Sesi otomatis menjadi Topik Materi.</p>
        </div>
        <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_ecourse" value="1" class="sr-only peer">
