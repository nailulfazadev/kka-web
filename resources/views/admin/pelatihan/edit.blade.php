...
<form method="POST" action="{{ route('admin.pelatihan.update', $training) }}" enctype="multipart/form-data" class="max-w-2xl space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div><label class="text-sm font-medium text-on-surface-variant mb-1 block">Mulai (Opsional)</label><input type="date" name="start_date" value="{{ old('start_date', $training->start_date?->format('Y-m-d')) }}" class="w-full bg-surface-container-highest border-none rounded-xl py-3 px-4 text-on-surface"></div>
        <div><label class="text-sm font-medium text-on-surface-variant mb-1 block">Selesai (Opsional)</label><input type="date" name="end_date" value="{{ old('end_date', $training->end_date?->format('Y-m-d')) }}" class="w-full bg-surface-container-highest border-none rounded-xl py-3 px-4 text-on-surface"></div>
    </div>
    ...
    <div class="glass-card border border-blue-500/20 bg-blue-500/5 rounded-xl p-4 flex items-center justify-between mb-4">
        <div>
        ...
        <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_ecourse" value="1" class="sr-only peer" @checked($training->is_ecourse)>
