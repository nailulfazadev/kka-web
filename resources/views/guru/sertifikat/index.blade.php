        ...
        <div class="glass-card rounded-2xl border border-white/5 p-5">
            ...
            <div class="flex flex-wrap gap-2 mt-4">
                @if($training->isDonasi() && !$reg->facility_paid)
                    <a href="{{ route('guru.pelatihan.show', $reg->id) }}" class="inline-flex items-center gap-2 bg-yellow-500/10 text-yellow-500 px-4 py-2 rounded-xl text-xs font-bold hover:bg-yellow-500/20"><i class="fa-solid fa-hand-holding-heart"></i> Donasi untuk Buka Fasilitas</a>
                    @if(!$training->facilities_released)
                        <span class="text-[10px] items-center flex text-outline self-center px-2 py-1 border border-white/5 rounded-lg">Belum Tersedia</span>
                    @endif
