...
<section class="px-6 pt-6">
    ...
    <div class="grid {{ $training->whatsapp_link ? 'grid-cols-1' : 'grid-cols-1' }} gap-3 mt-6 mb-4">
    ...
    {{-- Fasilitas Pelatihan Area --}}
    @if($training->isDonasi() || $training->facilities_released)
    <h3 class="text-lg font-bold text-on-surface mb-3 mt-8">Fasilitas Pelatihan</h3>
    @endif

    {{-- 1. Donation Block (Always visible for Donasi type if not paid, or shows pending if paid but facilities not released) --}}
    @if($training->isDonasi())
        @if(!$registration->facility_paid)
            <div class="glass-card p-6 rounded-[2rem] border border-white/5 mb-8 text-center bg-gradient-to-br from-surface-container-high/50 to-surface-container-low/50">
                <div class="w-16 h-16 rounded-full bg-yellow-500/10 flex items-center justify-center text-yellow-500 mx-auto mb-4 border border-yellow-500/20">
                    <i class="fa-solid fa-hand-holding-heart text-3xl"></i>
                </div>
                <h4 class="text-xl font-bold text-on-surface mb-2">Buka Akses Fasilitas (Donasi)</h4>
                <p class="text-[13px] text-on-surface-variant mb-6 max-w-sm mx-auto leading-relaxed">Dukung operasional kami dengan donasi sukarela. Anda akan langsung mendapatkan akses ke Sertifikat 32JP, Undangan, dan Materi Google Drive @if(!$training->facilities_released)saat sudah dibagikan oleh admin.@else selamanya.@endif</p>
                <form action="{{ route('guru.pelatihan.donasi', $registration->id) }}" method="POST" class="max-w-xs mx-auto">
                    @csrf
                    <div class="relative mb-4">
                        <span class="absolute left-5 top-1/2 -translate-y-1/2 text-on-surface-variant font-bold text-base">Rp</span>
                        <input type="number" name="amount" min="10000" step="1000" placeholder="Minimal 10.000" class="w-full bg-surface-container-highest border border-white/10 rounded-2xl py-4 pl-14 pr-4 text-on-surface focus:ring-2 focus:ring-yellow-500/50 text-left font-bold lg:text-lg outline-none shadow-inner" required>
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-r from-yellow-500 to-amber-600 text-white font-bold py-4 rounded-2xl hover:scale-[1.02] shadow-[0_8px_20px_rgba(234,179,8,0.25)] active:scale-95 transition-all">
                        <i class="fa-solid fa-bolt mr-2"></i> Lanjut Pembayaran
                    </button>
                </form>
            </div>
        @elseif(!$training->facilities_released)
            {{-- They paid, but admin hasn't released them yet --}}
            <div class="glass-card p-6 rounded-[2rem] border border-green-500/20 mb-8 text-center bg-green-500/5">
                <div class="w-16 h-16 rounded-full bg-green-500/20 flex items-center justify-center text-green-500 mx-auto mb-4 border border-green-500/30">
                    <i class="fa-solid fa-clock-rotate-left text-3xl"></i>
                </div>
                <h4 class="text-xl font-bold text-on-surface mb-2">Terima Kasih atas Donasi Anda! 🎉</h4>
                <p class="text-[13px] text-on-surface-variant max-w-sm mx-auto leading-relaxed">Fasilitas sedang dipersiapkan oleh tim kami. Sertifikat, Undangan, dan Materi akan otomatis muncul di sini segera setelah Admin membagikannya.</p>
            </div>
        @endif
    @endif

    {{-- 2. Actual Facilities (Only if released AND (not donasi OR already paid)) --}}
